export const StoryFiller = ({ loading }) => {
  return (
    <div
      className={
        loading
          ? "animate-pulse shadow rounded-md pl-4 md:max-w-md lg:max-w-xl w-full"
          : "shadow rounded-md pl-4 md:max-w-md lg:max-w-xl w-full"
      }
    >
      <div className=" flex space-x-4">
        <div className="rounded-full bg-stone-700 h-10 w-10"></div>
        <div className="flex-1 space-y-6 py-1">
          <div className="h-2 bg-stone-700 rounded"></div>
          <div className="space-y-3">
            <div className="grid grid-cols-3 gap-4">
              <div className="h-2 bg-stone-700 rounded col-span-2"></div>
              <div className="h-2 bg-stone-700 rounded col-span-1"></div>
            </div>
            <div className="h-2 bg-stone-700 rounded"></div>
          </div>
        </div>
      </div>
      <div className="flex space-x-4">
        {/* <div class="rounded-full bg-stone-700 h-10 w-10"></div> */}
        <div className="flex-1 space-y-6 py-1">
          <div className="h-2 bg-stone-700 rounded"></div>
          <div className="space-y-3">
            <div className="grid grid-cols-3 gap-4">
              <div className="h-2 bg-stone-700 rounded col-span-2"></div>
              <div className="h-2 bg-stone-700 rounded col-span-1"></div>
            </div>
            <div className="h-2 bg-stone-700 rounded"></div>
          </div>
        </div>
      </div>
      <div className="flex space-x-4">
        {/* <div class="rounded-full bg-stone-700 h-10 w-10"></div> */}
        <div className="flex-1 space-y-6 py-1">
          <div className="h-2 bg-stone-700 rounded"></div>
          <div className="space-y-3">
            <div className="grid grid-cols-3 gap-4">
              <div className="h-2 bg-stone-700 rounded col-span-2"></div>
              <div className="h-2 bg-stone-700 rounded col-span-1"></div>
            </div>
            <div className="h-2 bg-stone-700 rounded"></div>
          </div>
        </div>
      </div>
      <div className="flex space-x-4">
        {/* <div class="rounded-full bg-stone-700 h-10 w-10"></div> */}
        <div className="flex-1 space-y-6 py-1">
          {/* <div class="h-2 bg-stone-700 rounded"></div> */}
          <div className="space-y-3">
            <div className="grid grid-cols-3 gap-4">
              {/* <div class="h-2 bg-stone-700 rounded col-span-2"></div> */}
              <div className="h-2 bg-stone-700 rounded col-span-1"></div>
              <div className="h-2 bg-stone-700 rounded col-span-1"></div>
              <div className="h-2 bg-stone-700 rounded col-span-1"></div>
            </div>
            <div className="h-2 bg-stone-700 rounded"></div>
          </div>
        </div>
      </div>
    </div>
  );
};
